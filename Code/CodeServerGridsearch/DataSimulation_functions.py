...
def Balanced_EEG(params, relevant_brain_area, sig_noise_ratio = 0.9, channel_noise_ratio = 0.9, seed = 0, only_save = False):
    ...
    file_name = "./data_"  + str(relevant_brain_area) + "_" + str(seed) + ".pkl"
    if os.path.isfile(file_name):
        return pickle.load(open(file_name, "rb"))
    else:
        if os.path.isfile('previous_data_file.pkl'): 
            prev_file = pickle.load(open('previous_data_file.pkl', 'rb'))
            if os.path.isfile(prev_file):  os.remove(prev_file)    
    ...
    projection_matrix = pickle.load(open("./projection_matrix.pkl", "rb" ))
    ...
    pickle.dump(file_name, open('previous_data_file.pkl','wb'))
