...
def train_brain_area(brain_area, param_values, val_perc = 0.1, plot = False):
    ...
    plot_line([np.array(Validator_clas)], ['Classification Validation Loss'], title = 'Classification Validation Loss')

   
    return (mse_pred, mse_clas, truepositive_clas, truenegative_clas, STOP, STOP_clas)
#%%  
if __name__ == '__main__':
    optimal_hyper_params = {'nodes_pred': 200,
     'nodes_Conv_clas': 25,
     'nodes_Dense_clas': 10,
     'kernel_size': 2,
     'strides': 3,
     'learning_rate_pred': 1e-06,
     'learning_rate_clas': 1e-05,
     'batch_sizes_pred': 25,
     'batch_sizes_clas': 5,
     'val_treshold_pred': 1e-09,
     'val_treshold_clas': 1e-09,
     'max_val_amount_pred': 100,
     ...
     'time_steps': 100,
     'brain_areas': 1000}
    a, b, c, d, e, f = train_brain_area(231, results.iloc[339])
