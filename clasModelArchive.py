# -*- coding: utf-8 -*-
"""
Created on Thu Nov  5 09:44:31 2020
    ...
    for epoch in range(epochs):
        for block in tqdm(range(no_blocks)):
            ...
            for chunk in chunks(np.arange(prediction.shape[0]), chunk_size):
                Net.zero_grad()
                hey = predictions[chunk, :].view(-1, 1, timesteps)
                outputs = Net(hey)
                loss = loss_function(outputs, torch.Tensor(prediction_label[chunk].view(-1,1)))
                losses += [loss.detach().numpy().tolist()]
                loss.backward()
